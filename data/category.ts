...
export function getTopCategories(): Category[] {
  return categories.flatMap(categoryGroup => categoryGroup.items.filter(item => item.parent === null));
}

export function getCategories(parent: string): Category[] {
  return categories.flatMap(categoryGroup => categoryGroup.items.filter(item => item.parent === parent));
}
